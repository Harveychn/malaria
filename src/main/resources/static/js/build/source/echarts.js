    ...
    Toolbox.prototype = {
        ...
        _buildItem: function () {
            ...
            for (var i = 0; i < iconLength; i++) {
                ...
                if (this._featureIcon[this._iconList[i]]) {
                    itemShape.style.image = this._featureIcon[this._iconList[i]].replace(new RegExp('^images:\\/\\/'), '');
        ...
        __onSaveAsImage: function () {
            ...
            if (!this.myChart.isConnected()) {
                image = this.zr.toDataURL('images/' + imgType, this.option.backgroundColor && this.option.backgroundColor.replace(' ', '') === 'rgba(0,0,0,0)' ? '#fff' : this.option.backgroundColor);
            } else {
    ...
    Legend.prototype = {
        ...
        _getItemShapeByType: function (x, y, width, height, color, itemType, defaultColor) {
            ...
            if (itemType.match('image')) {
                var imageLocation = itemType.replace(new RegExp('^images:\\/\\/'), '');
                itemType = 'image';
            }
            switch (itemType) {
            case 'line':
